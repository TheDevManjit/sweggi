...
export default function Header() {
    ...
    return (
        <>
                ...
                <div className='max-w-[1200px] mx-auto flex items-center'>
                    ...
                    <div className='flex items-center'>
                        <p className='text-gray-500'>
                            <span className='border-b-[3px] text-black border-black font-bold pb-1'>Bakarganj</span> Sabzibagh, Bakarganj, Patna,
                        </p>
                    ...
                    <nav className='flex list-none gap-10 ms-auto font-[18px] font-semibold'>
                        {links.map((link) => (
                            <li className='flex items-center gap-3 hover:text-[#ff5200] cursor-pointer' key={link.id}>
                                {link.id === 5 ? (
                                    <NavLink to="/cart" className='flex items-center gap-3 hover:text-[#ff5200] cursor-pointer'>
                                        {link.icon}
                                        {link.name}
                                        <sup>{link.sup}</sup>
                                    </NavLink>
            ...
            {/* Mobile Header */}
            <header ref={myComponentRef} className={`${isMobile ? '' : 'hidden'}`}>
                <div className='flex justify-between px-3 my-2 items-center'>
                    ...
                    <div className='bg-[#373A40] rounded-full flex justify-between items-center p-3 text-white'>
                        <FaRegUser onClick={showRightMenu} />
                    </div>
                </div>
